...
class BetterPlayerController {
  ...
  ///Selected VideoPlayerValueUpdated when error occurred.
  VideoPlayerValueUpdated? _VideoPlayerValueUpdatedOnError;
  ...
  void _onVideoPlayerChanged() async {
    final VideoPlayerValueUpdated currentVideoPlayerValueUpdated =
        videoPlayerController?.value ??
            VideoPlayerValueUpdated(duration: const Duration());

    if (currentVideoPlayerValueUpdated.hasError) {
      _VideoPlayerValueUpdatedOnError ??= currentVideoPlayerValueUpdated;
      _postEvent(
        BetterPlayerEvent(
          BetterPlayerEventType.exception,
          parameters: <String, dynamic>{
            "exception": currentVideoPlayerValueUpdated.errorDescription
          },
        ),
      );
    }
    if (currentVideoPlayerValueUpdated.initialized &&
    ...
    if (currentVideoPlayerValueUpdated.isPip) {
      _wasInPipMode = true;
    ...
    if (_betterPlayerSubtitlesSource?.asmsIsSegmented == true) {
      _loadAsmsSubtitlesSegments(currentVideoPlayerValueUpdated.position);
    }
    ...
    if (now - _lastPositionSelection > 500) {
      ...
      _postEvent(
        BetterPlayerEvent(
          ...
          parameters: <String, dynamic>{
            _progressParameter: currentVideoPlayerValueUpdated.position,
            _durationParameter: currentVideoPlayerValueUpdated.duration
          },
        ),
      );
    }
  ...
  void _handleVideoEvent(VideoEvent event) async {
    switch (event.eventType) {
      ...
      case VideoEventType.completed:
        final VideoPlayerValueUpdated? videoValue =
            videoPlayerController?.value;
        _postEvent(
          BetterPlayerEvent(
  ...
  Future retryDataSource() async {
    ...
    if (_VideoPlayerValueUpdatedOnError != null) {
      final position = _VideoPlayerValueUpdatedOnError!.position;
      await seekTo(position);
      await play();
      _VideoPlayerValueUpdatedOnError = null;
    }
  }
